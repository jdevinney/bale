...
#else
static void ranf_array(aa,n)    /* put n new random fractions in aa */
...
#ifdef __STDC__
static void ranf_start(long seed)
#else
static void ranf_start(seed)    /* do this before using ranf_array */
...
static inline double ranf_arr_next() {
  if (*ranf_arr_ptr >= 0) {
    return *ranf_arr_ptr++;
  } else { 
